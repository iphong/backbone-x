...
var _set2 = __webpack_require__(85);

var _set3 = _interopRequireDefault(_set2);

var _mapValues2 = __webpack_require__(118);
...
function Compute(deps, options) {
	...
	if (_underscore2.default.isArray(deps) && _underscore2.default.isFunction(options)) options = { deps: deps, get: options };else if (_underscore2.default.isFunction(deps)) options = _underscore2.default.defaults({ get: deps }, options);else options = deps || {};

	_underscore2.default.defaults(this, options, {
		deps: [],
		init: function () {
			return null;
		},
...
function Observable(attrs) {
	const map = obj => {
		return _underscore2.default.mapObject(obj, (value, key) => {
			if ((0, _getPrototypeOf2.default)(value) === Object.prototype) {
...
function Model(attrs = {}, options = {}) {
	if (!(this instanceof Model)) {
		if (isPrototypeOf(attrs, Model)) return attrs.create.apply(attrs, (0, _underscore2.default)(arguments).slice(1));
		return Model.create.apply(Model, arguments);
	}
	const localStorageKey = options.localStorageKey;
	options.localStorageKey = void 0;
	_underscore2.default.extend(this, _underscore2.default.pick(options, '_parent', '_relatedKey'));
	_underscore2.default.each(this.computes, this._registerComputeValue, this);
	BackboneModel.apply(this, arguments);

	this.proxy = this.getProxy();
	if (!_underscore2.default.isUndefined(localStorage) && !_underscore2.default.isUndefined(localStorageKey)) {
		...
		this.on('all', _underscore2.default.debounce(() => {
			localStorage.setItem(localStorageKey, (0, _stringify2.default)(this.toJSON()));
		}, 1000));
	}
...
(0, _setPrototypeOf2.default)(Model.prototype, BackboneModel.prototype);

// prototypes
_underscore2.default.extend(Model.prototype, {
	relations: {},
	computes: {},
	defaults: {},
	...
	getProxy() {
		return new Proxy(this.attributes, {
			...
			getPrototypeOf: target => {
				return (0, _getPrototypeOf2.default)(this);
			},
	...
	subscribe(events, handler, context) {
		...
		this.on('change', () => {
			const changes = (0, _keys2.default)(this.changed);
			const matched = _underscore2.default.intersection(keys, changes).length;
			if (matched) {
	...
	get: function (key) {
		if (!_underscore2.default.isString(key)) return;
		let value = this;
		const regex = /(\w+)(?:#(\w+))?/g;
		let match;
		while (match = regex.exec(key)) {
			value = value instanceof BackboneModel ? getComputedValue(value.$model || value, match[1]) : typeof value === 'object' ? value[match[1]] : undefined;
	...
	setRelation: function (attr, val, options) {
		...
		if (this.relations && _underscore2.default.has(this.relations, attr)) {
			...
			if (relation && relation instanceof Collection) {
				...
				if (val instanceof Collection || val instanceof Array) {
					val = val.models || val;
					modelsToAdd = _underscore2.default.clone(val);
					...
					relation.each(function (model, i) {
						...
						const rModel = _underscore2.default.find(val, function (_model) {
					...
					_underscore2.default.each(modelsToRemove, function (model) {
	...
	set: function (key, val, options) {
		...
		if (typeof key === 'string') {
			return this._set((0, _set3.default)({}, key, val), options);
		}
		// if (typeof key === 'string') {
	...
	_set: function (key, val, options) {
		...
		for (attr in attrs) {
			...
			if (_underscore2.default.isUndefined(val)) continue;
	...
	toJSON: function () {
		let attr;
		const obj = {};
		const attrs = _underscore2.default.extend({}, this.attributes);
		for (const key in attrs) {
			attr = this.get(key);
			if (attr instanceof BackboneModel || attr instanceof BackboneCollection) attr = (attr.$model || attr).toJSON();else if (_underscore2.default.isObject(attr)) {
				const proto = (0, _getPrototypeOf2.default)(attr);
				if (proto === Array.prototype || proto === Object.prototype) {
					attr = _underscore2.default.clone(attr);
				} else {
					attr = void 0;
				}
			}
			if (!_underscore2.default.isUndefined(attr)) {
				obj[key] = attr;
			}
		}
		return obj;
	...
	toCompactJSON: function () {
		...
		for (const key in this.attributes) {
			if (this.attributes.hasOwnProperty(key)) {
				...
				if (_underscore2.default.isEqual(attr, this.defaults[key])) continue;
	...
	_triggerParentChange: function (options) {
		...
		parent.changed = {};
		_underscore2.default.extend(options, { chained: true });
	...
	_registerComputeValue: function (compute, attr) {
		_underscore2.default.each(compute.deps, function (depAttr) {
			this.on(`change:${depAttr}`, function (model, value, options) {
				...
				if (value instanceof BackboneModel || value instanceof Collection) {
					...
					_underscore2.default.each(value.changed, function (subValue, subAttr) {
...
// statics
_underscore2.default.extend(Model, {
	create: function (attrs, protos, statics) {
		var protos = protos || {};
		var statics = statics || {};
		const defaults = _underscore2.default.extend({}, this.prototype.defaults, protos.defaults, attrs);
		const relations = _underscore2.default.extend({}, this.prototype.relations, protos.relations);
		const computes = _underscore2.default.extend({}, this.prototype.computes, protos.computes);
		...
		const ExtendedModel = this.extend(_underscore2.default.extend({}, protos, {
			defaults: defaults,
			relations: relations,
			computes: computes
		}), _underscore2.default.extend({}, statics, {
			create: Model.create,
			extend: Model.extend,
			define: Model.define
		}));
...
function Collection(models, options) {
	if (!(this instanceof Collection)) {
		if (isPrototypeOf(models, Collection)) return models.create.apply(models, (0, _underscore2.default)(arguments).slice(1));
...
(0, _setPrototypeOf2.default)(Collection.prototype, BackboneCollection.prototype);

// prototypes
_underscore2.default.extend(Collection.prototype, {
	model: Model,
	...
	_triggerParentChange: function (model, options) {
		...
		if (model && model.collection) {
			...
			for (const key in model.changed) {
				if (!_underscore2.default.isUndefined(modelID)) {
	...
	resetRelations: function (options) {
		_underscore2.default.each(this.models, function (model) {
			_underscore2.default.each(model.relations, function (rel, key) {
	...
	reset: function (models, options) {
		...
		this._reset();
		this.add(models, _underscore2.default.extend({ silent: true }, options));
		if (!options.silent) {
	...
	toCompactJSON: function () {
		const models = (0, _underscore2.default)(this.models).map(function (model) {
...
// statics
_underscore2.default.extend(Collection, {
	create: function (models, protos, statics) {
		var statics = _underscore2.default.extend({}, statics, {
			create: Collection.create,
			extend: Collection.extend,
			define: Collection.define
		});
		if (_underscore2.default.isArray(models) || isPrototypeOf(models, Model)) return Collection.extend(_underscore2.default.extend({}, protos, {
			model: _underscore2.default.isArray(models) ? models[0] : models
		}), statics);else if (_underscore2.default.isObject(models)) return Collection.extend(_underscore2.default.extend({}, protos, {
			relations: models
...
function getComputedValue(model, key) {
	if (model.computes && model.computes[key]) {
		...
		const deps = (0, _underscore2.default)(compute.deps).map(function (dep) {
			return model.get(dep);
		});
		if (_underscore2.default.isFunction(compute.get)) {
...
function setPrototypeOf(child, prototype) {
	if (_underscore2.default.isFunction(_setPrototypeOf2.default)) (0, _setPrototypeOf2.default)(child.prototype || child, prototype);else (child.prototype || child).__proto__ = prototype;
	return child;
}
...
function cb(value, context, argCount) {
	if (value == null) return _underscore2.default.identity;
	if (_underscore2.default.isFunction(value)) return optimizeCb(value, context, argCount);
	if (_underscore2.default.isObject(value)) return _underscore2.default.matcher(value);
	return _underscore2.default.property(value);
}
