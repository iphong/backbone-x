...
const BackboneExtend = Backbone.Model.extend
...
export const Collection = (function() {
	...
	_.extend(Collection.prototype, {
		...
		_triggerParentChange: function(model, options) {
			...
			if (model && model.collection) {
				...
				for (const key in model.changed) {
					...
					if (!_.isUndefined(model.id)) {
						parent.changed[
							`${this._relatedKey}#${model.id}.${key}`
							] =
							model.changed[key]
						parent.trigger(
							`change:${this._relatedKey}#${model.id}.${key}`,
							parent,
							model.changed[key],
							options
						)
					...
					parent.changed[
						`${this._relatedKey}[${modelIndex}].${key}`
						] =
						model.changed[key]
					parent.trigger(
						`change:${this._relatedKey}[${modelIndex}].${key}`,
						parent,
						model.changed[key],
						options
					)
			...
			if (!_.isUndefined(model.id)) {
				parent.trigger(
					`change:${this._relatedKey}#${model.id}`,
					parent,
					options
				)
			}
